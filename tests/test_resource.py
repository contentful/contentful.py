        ...
        resource = Resource(
            {
                "sys": {
                    "space": {
                        "sys": {"type": "Link", "linkType": "Space", "id": "foo"}
                    },
                    "contentType": {
                        "sys": {"type": "Link", "linkType": "ContentType", "id": "bar"}
                    },
                    "type": "Entry",
...
class FieldsResourceTest(TestCase):
    def test_fields_resource(self):
        resource = FieldsResource(
            {
                ...
                "fields": {"foo": "bar", "baz": 123, "qux": True},
            }
        )
        ...
        self.assertEqual(
            resource.fields("foo-locale"), {"foo": "bar", "baz": 123, "qux": True}
        )
    ...
    def test_pickleable_resource(self):
        ...
        self.assertEqual(
            resource.fields("foo-locale"), deserialized_resource.fields("foo-locale")
        )
        self.assertEqual(
            resource.fields("bar-locale"), deserialized_resource.fields("bar-locale")
        )
    ...
    def test_fields_resource_localized(self):
        ...
        self.assertEqual(
            resource.fields("foo-locale"), {"foo": "bar-foo", "baz": 123, "qux": True}
        )
        self.assertEqual(
            resource.fields("bar-locale"), {"foo": "bar-bar", "baz": 456, "qux": False}
        )
...
class LinkTest(TestCase):
    def test_link(self):
        link = Link({"sys": {"type": "Link", "linkType": "Space", "id": "foo"}})
    ...
    def test_link_space_resource(self):
        with vcr.use_cassette("fixtures/link/resolve_space.yaml"):
            link = Link(
                {"sys": {"type": "Link", "linkType": "Space", "id": "cfexampleapi"}}
            )
    ...
    def test_link_other_resource(self):
        with vcr.use_cassette("fixtures/link/resolve_other.yaml"):
            link = Link(
                {"sys": {"type": "Link", "linkType": "ContentType", "id": "cat"}}
            )
            client = Client("cfexampleapi", "b4c0n73n7fu1", content_type_cache=False)
