...
API reference docs are built with [Sphinx](https://www.sphinx-build.org/) and committed as static HTML under `docs/`. The source lives in `_docs/`. Everything directly under `docs/` is auto-generated and must not be hand-edited, with one exception: `docs/ADRs/`, which holds this repository's decision records and is deliberately excluded from the build's prune step.
...
4. Empty every entry directly under `docs/` except `docs/ADRs/`, then copy built HTML from `_docs/_build/html/` → `docs/`
